    ...
    public class JwtMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            try
            {
                ...
                UserTokenResponse AuthInfo = await _tokenServices.GetInfoFromToken(token);
                if (AuthInfo == null)
                {
                    context.Response.StatusCode = StatusCodes.Status404NotFound;
                    await context.Response.WriteAsJsonAsync(new { Message = "Access Token không tồn tại hoặc đã bị xóa" });
                    return;
                }
                if (AuthInfo.ExpiryDate < dateTimeNow)
                {
                    context.Response.StatusCode = StatusCodes.Status401Unauthorized;
                    await context.Response.WriteAsJsonAsync(new { Message = "Access Token đã hết hạn, vui lòng kiểm tra lại" });
                    return;
                }

                var refresh_token_old = await _tokenServices.GetRefreshToken(AuthInfo.Id);
                UserTokenResponse AuthRefreshToken = await _tokenServices.GetInfoFromToken(refresh_token_old);
