...
namespace Domain.Services
{
    public class TokenServices : BaseService, ITokenServices
    {
        ...
        public async Task<UserTokenResponse> GetInfoFromToken(string token)
        {
        ...
        {
            var authHeader = _HttpContextHelper!.GetHeader("Authorization");
            var userMeToken = !string.IsNullOrEmpty(authHeader) ? GetInfoFromToken(authHeader).GetAwaiter().GetResult() : null;
            return userMeToken;
        }
        ...
        {
            var user = await _user.FindAsync(f => f.Id == info.UserId);
            if(user == null)
                return HttpResponse.Error("Người dùng không tồn tại.", System.Net.HttpStatusCode.NotFound);

            var refreshToken = await _token.FindAsync(x => x.UserId == info.UserId);
